...
class HomeScreenWidgetsController private constructor(context: Context) {

    private val helper: DatabaseOpenHelper = DatabaseOpenHelper.getInstance(context)

    fun getAll(): ArrayList<HomeScreenItem<*>> {
        return helper.writableDatabase.select(tableName).parseList(classParser<HomeScreenItem<*>>()) as ArrayList<HomeScreenItem<*>>
    }
    ...
    companion object : SingletonHolder<HomeScreenWidgetsController, Context>(::HomeScreenWidgetsController) {
        const val tableName: String = "HomeScreenWidgets"
